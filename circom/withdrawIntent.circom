...
template WithdrawIntentCircuit() {
    ...
    // Public input - hash of the intent (provided by user, verified in circuit)
    signal input IntentHash;

    // Compute and verify intent hash: hash(amount, destination, tokenId, nonce)
    component intentHasher = Poseidon(4);
    intentHasher.inputs[0] <== ValueToWithdraw;
    intentHasher.inputs[1] <== Destination;
    intentHasher.inputs[2] <== TokenId;
    intentHasher.inputs[3] <== Nonce;

    // Verify the provided hash matches the computed hash
    IntentHash === intentHasher.out;
...
component main { public [ SenderPublicKey, SenderBalanceC1, SenderBalanceC2, AuditorPublicKey, AuditorPCT, AuditorPCTAuthKey, AuditorPCTNonce, IntentHash ] } = WithdrawIntentCircuit();
